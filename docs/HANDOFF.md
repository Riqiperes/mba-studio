...
7. **Implementar botón "Enviar recordatorio" waitlist** en admin (`ClassBookingsPage`) — pospuesto explícitamente por el usuario hasta que se implemente WhatsApp real.
8. ~~Campos médicos~~ — **revertido y movido**. Se habían puesto en `dependents` (alumnos); el usuario pidió moverlos a `profiles` (cliente adulto) en vez de alumnos, porque tiene más sentido para Pilates (embarazo/hernia/lesiones) en el adulto. Migración `022`: agrega `profiles.medical_conditions`/`notes`, elimina `dependents.medical_conditions`/`notes`/`age` (la edad de un alumno ya no se guarda como número, se calcula en el momento desde `birth_date`, que ya existía). Editable en `CustomerDetailPage`.

### Rediseño grande de `apps/admin` (mismo día, pedido explícito del usuario)

- **Nav**: reemplazado el listado plano por 2 dropdowns hover (`Estudio`: Paquetes/Instructores/Clientes/Clases, `Academia`: Ver lista de alumnos/Ver lista de horarios) + `Colegiaturas` y `Usuarios` como items sueltos. `AdminLayout.tsx`.
- **Lista de alumnos** (`StudentsPage.tsx`, ruta `/students`, ahora "Ver lista de alumnos" en el nav): rediseñada por completo. Cada fila: nombre, tutor, edad (calculada de `birth_date`, ya no es un campo guardado), tintada verde si su colegiatura del mes en curso está `PAGADO`, rojo claro si no. Sin botones de acción ni columna de estado — click en la fila abre `DependentFormModal` (editar datos + activar/desactivar movido dentro del modal). Nuevo servicio `academyTuitionService.listCurrentMonthPaymentStatus()`.
- **Colegiatura, más simple**: `MarkPaymentModal` ya no tiene selector de "periodo" (confundía porque solo mostraba los periodos ya vencidos calculados desde la fecha de inscripción). Ahora es un `<input type="month">` libre — cualquier mes, incluidos meses pasados, sin las etiquetas raras de antes. De paso se encontró y corrigió un bug real: el modal se renderizaba SIEMPRE (sin ningún `open`/guardia condicional) en `AcademyGroupDetailPage` y `AcademyOverduePage` — ahora ambos lo envuelven en `{selectedX && (...)}`.
...
9. ~~Rol INSTRUCTOR_ADMIN~~ — hecho, con brainstorming previo (ver conversación). Alcance final, más grande que lo original porque el usuario pidió resolver de una vez la gestión general de roles:
   ...
   - **`AdminLayout`**: nav condicional — `INSTRUCTOR_ADMIN` ve solo "Mis clases"; `BUSINESS_ADMIN`/`SUPER_ADMIN` ven además "Usuarios"; `STAFF` ve el nav de siempre.
   - **`/instructor/my-classes`** nueva: sus clases de Studio (roster de reservados) + sus grupos de Academia (roster de inscritos + alerta médica). Todo de solo lectura — no expone botones de reservar/cancelar/promover (esas RPCs rechazarían al instructor de todas formas).
   - **RLS faltante encontrada**: `academy_groups` nunca tuvo policy de lectura para `INSTRUCTOR_ADMIN` (`016` solo cubrió `academy_enrollments`) — un instructor podía ver los inscritos de su grupo pero no el grupo en sí. Agregada en `021`.
   - **Bootstrap de roles**: confirmado que ya existe el mecanismo correcto (`admin_allowed_emails`, `009`) para dar de alta el primer `SUPER_ADMIN`/`BUSINESS_ADMIN`/`STAFF` de una cuenta que todavía no se registra — insertar una fila ahí con su email antes de que se registre. El usuario ya está sembrado como `SUPER_ADMIN` (su propio email). Pendiente: dar de alta a la prima del usuario cuando dé su email (dijo "todavía no").
   - **Nota:** `database.types.ts` (admin) se actualizó a mano con el tipo de `list_business_profiles()` porque la regeneración completa vía MCP estaba bloqueada por el token-optimizer de esta sesión (falso positivo de "llamada duplicada" — el schema sí había cambiado). Recomendado: correr `generate_typescript_types` en una sesión limpia para confirmar que coincide.
