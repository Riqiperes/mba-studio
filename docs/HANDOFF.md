...
8. ~~Campos médicos~~ — hecho: `age`/`medicalConditions`/`notes` en `DependentFormModal` (create+edit, ambos flujos: `CustomerDetailPage` y `StudentsPage`), alerta ⚠️ (title = texto de la condición) en `DependentsTable`, en la tabla de inscritos de `AcademyGroupDetailPage`, y en `InstructorMyClassesPage` (punto 9).
